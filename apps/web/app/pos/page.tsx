...
interface PendingOrder { id:string; type:string; status:string; total:number; paid:number; createdAt:string; tableNumber:number|null; tableName:string|null; itemCount:number; }
...
export default function FOHPage() {
  ...
  const subtotal=useMemo(()=>lines.reduce((s,l)=>s+(l.unitPrice+l.modifiers.reduce((m,x)=>m+x.priceDelta,0))*l.quantity,0),[lines]);
  ...
  function addItem(item:MenuItemDTO){if(item.modifierGroups.length){setModifierItem(item);return}setLines(p=>[...p,makeLine(item.id,item.name,item.price,item.printerStation,[])]);}
  function addDeal(deal:DealDTO){setLines(p=>[...p,{lineId:uid(),menuItemId:"",name:deal.name,unitPrice:deal.type==="PERCENT"?0:deal.value,quantity:1,notes:deal.type==="PERCENT"?`${deal.value}% off order`:"",seat:null,station:"MAIN",modifiers:deal.items.map(i=>({id:i.id,name:`${i.quantity}× ${i.name}`,priceDelta:0}))}]);}
  ...
  async function openEdit(id:string){try{const r=await fetch(`/api/orders/${id}`);const d=await r.json();const o=d.order;const loaded:CartLine[]=(o.items??[]).map((it:any)=>({lineId:it.id,menuItemId:it.menuItemId??"",name:it.name,unitPrice:it.unitPrice,quantity:it.quantity,notes:it.notes??"",seat:it.seat,station:it.station,modifiers:(it.modifiers??[]).map((m:any)=>({id:m.id,name:m.name,priceDelta:m.priceDelta}))}));setEditOrderId(id);setEditLines(loaded);setEditOriginalIds(loaded.map(l=>l.lineId));setEditOriginal(Object.fromEntries(loaded.map(l=>[l.lineId,{quantity:l.quantity,notes:l.notes}])));const paid=(o.payments??[]).reduce((s:number,p:any)=>s+p.amount,0);setEditMeta({type:o.type,tableNumber:o.table?.number??null,paid,subtotal:o.subtotal??0,tax:o.tax??0,total:o.total});setShowPending(false)}catch{showToast("Could not load order")}}
  async function sendUpdate(){if(!editOrderId)return;const original=new Set(editOriginalIds);const current=new Set(editLines.map(l=>l.lineId));const removed=editOriginalIds.filter(id=>!current.has(id));const added=editLines.filter(l=>!original.has(l.lineId));const updated=editLines.filter(l=>original.has(l.lineId)).filter(l=>editOriginal[l.lineId]&&(editOriginal[l.lineId].quantity!==l.quantity||editOriginal[l.lineId].notes!==l.notes)).map(l=>({id:l.lineId,quantity:l.quantity,notes:l.notes}));try{await fetch(`/api/orders/${editOrderId}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({addItems:added.length?linesToItems(added):undefined,updateItems:updated.length?updated:undefined,removeItemIds:removed.length?removed:undefined})});showToast("Order updated");setEditOrderId(null);setEditLines([]);await loadPending()}catch{showToast("Could not update order")}}
  async function submitEditPayment(result:PayResult){if(!payExisting)return;try{await fetch(`/api/orders/${payExisting.orderId}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({payments:result.payments,discountPaisa:result.discountPaisa,discountReason:result.discountReason})});setPayOpen(false);setPayExisting(null);setEditOrderId(null);setEditLines([]);await loadPending();showToast("Payment recorded");window.open(`/bill/${payExisting.orderId}`,"_blank")}catch(e){throw e}}
  async function voidOrder(id:string){try{await fetch(`/api/orders/${id}/void`,{method:"POST"});setPendingVoidId(null);await loadPending();showToast("Order voided")}catch{showToast("Could not void order")}}
  ...
  return <SecurityLock timeoutMinutes={5}><div className="h-screen flex flex-col bg-background overflow-hidden">
    <header className="h-[68px] shrink-0 bg-white border-b border-line flex items-center justify-between px-5">
    ...
    <main className="flex-1 min-h-0 grid grid-cols-[240px_minmax(0,1fr)_360px] gap-3 p-3">
      ...
      <aside className="min-h-0 bg-white border border-line rounded-xl flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="border-t border-line p-4 bg-surface-2"><div className="space-y-2 text-sm"><div className="flex justify-between text-muted"><span>Subtotal</span><span>{formatPaisa(editing?editMeta?.subtotal??0:subtotal,currency)}</span></div>{company?.gstEnabled?<div className="flex justify-between text-muted"><span>GST {company.gstRate}%</span><span>{formatPaisa(editing?editMeta?.tax??0:tax,currency)}</span></div>:null}<div className="pt-2 mt-1 border-t border-line flex justify-between items-end"><span className="font-bold">Total</span><span className="text-2xl font-extrabold text-brand">{formatPaisa(editing?editMeta?.total??0:total,currency)}</span></div></div>
          ...
          {editing?<button onClick={()=>setPayExisting({orderId:editOrderId!,existingPaid:editMeta?.paid??0,subtotal:editMeta?.subtotal??0,tax:editMeta?.tax??0,total:editMeta?.total??0})} className="w-full mt-2 h-10 rounded-lg border border-brand/30 bg-brand-soft text-brand font-bold text-sm">Record payment</button>:activeOrderId?<button onClick={()=>window.open(`/bill/${activeOrderId}`,"_blank")} className="w-full mt-2 h-9 text-xs font-semibold text-muted hover:text-brand flex items-center justify-center gap-2"><PosIcon name="print" size={14}/> Reprint last receipt</button>:null}
        </div>
      </aside>
    </main>
    ...
    {showPending?<PendingModal orders={pending} currency={currency} onClose={()=>setShowPending(false)} onEdit={openEdit} onPay={o=>setPayExisting({orderId:o.id,existingPaid:o.paid,subtotal:o.total,tax:0,total:o.total})} onVoid={setPendingVoidId} onPrint={id=>window.open(`/bill/${id}`,"_blank")}/>:null}
    {pendingVoidId?<ManagerPinModal title="Void order" confirmLabel="Void" onSuccess={()=>voidOrder(pendingVoidId)} onClose={()=>setPendingVoidId(null)}/>:null}
